...
export function MonteCarloSimulator() {
  ...
  const runMonteCarlo = () => {
    if (!window.Chart) {
      alert("Chart.js ainda está carregando. Tente novamente em alguns segundos.")
      return
    }
    ...
    if (curvesChartRef.current) {
      curvesChartInstance.current = new window.Chart(curvesChartRef.current, {
        ...
        options: {
          ...
          scales: {
            x: { title: { display: true, text: "Número de Trades", color: "#FFF" }, ticks: { color: "#FFF" } },
            y: { title: { display: true, text: "Capital (R$)", color: "#FFF" }, ticks: { color: "#FFF" } },
          },
        },
      })
    }
    ...
    if (histChartRef.current) {
      histChartInstance.current = new window.Chart(histChartRef.current, {
        ...
        data: {
          labels: histLabels,
          datasets: [{ label: "Frequência", data: counts, backgroundColor: "#BBF717" }],
        },
        options: {
          responsive: true,
          scales: {
            x: { title: { display: true, text: "Capital Final (R$)", color: "#FFF" }, ticks: { color: "#FFF" } },
            y: { title: { display: true, text: "Frequência", color: "#FFF" }, ticks: { color: "#FFF" } },
          },
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-400">Simule cenários futuros baseados em estatísticas</p>
      </div>
      ...
      <div className="bg-[#2A2B2A] p-5 mb-5 rounded">
        <div className="flex flex-wrap gap-5 mb-5">
          <div className="flex-1 min-w-[200px]">
            <label className="block mb-2 font-bold">Valor Inicial da Carteira (R$)</label>
            <Input
              type="number"
        ...
        <div className="flex flex-wrap gap-5">
          <div className="flex-1 min-w-[200px]">
            <label className="block mb-2 font-bold">Número de Trades por Simulação</label>
            <Input
              type="number"
              value={numTrades}
      ...
      {results && (
        <div className="space-y-2.5 mb-5">
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Média Capital Final:</strong> {formatCurrency(results.meanCap)}
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Média P/L:</strong>{" "}
            <span className={results.meanPnl < 0 ? "text-[#FF4C4C]" : "text-[#BBF717]"}>
              {results.meanPnl >= 0 ? "+" : ""}
              {formatCurrency(results.meanPnl)}
            </span>
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Mediana Capital Final:</strong> {formatCurrency(results.medianCap)}
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Mediana P/L:</strong>{" "}
            <span className={results.medianPnl < 0 ? "text-[#FF4C4C]" : "text-[#BBF717]"}>
              {results.medianPnl >= 0 ? "+" : ""}
              {formatCurrency(results.medianPnl)}
            </span>
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Melhor Capital Final:</strong> {formatCurrency(results.bestCap)}
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Melhor P/L:</strong> <span className="text-[#BBF717]">+{formatCurrency(results.bestPnl)}</span>
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Pior Capital Final:</strong> {formatCurrency(results.worstCap)}
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Pior P/L:</strong> <span className="text-[#FF4C4C]">{formatCurrency(results.worstPnl)}</span>
          </div>
          <div className="bg-black p-2.5 rounded-sm">
            <strong>Drawdown Máximo:</strong> {formatCurrency(results.globalMaxDrawdown)}
          </div>
        </div>
      )}
      ...
      <div className="space-y-5">
        <div className="bg-[#2A2B2A] p-5 rounded">
          <h2 className="text-xl font-bold text-[#BBF717] mb-3">Curvas de Equity (amostragem)</h2>
        ...
        <div className="bg-[#2A2B2A] p-5 rounded">
          <h2 className="text-xl font-bold text-[#BBF717] mb-3">Distribuição de Capital Final</h2>
          <div className="bg-black p-4 rounded" style={{ height: "400px" }}>
